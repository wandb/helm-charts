...
{{- else if .Values.global.bufstream.enabled -}}
kafka://$(KAFKA_BROKER_HOST):9092/$(KAFKA_TOPIC_RUN_UPDATE_SHADOW_QUEUE)?consumer_group_id={{ .Values.kafka.consumerGroupId }}&num_partitions=$(KAFKA_RUN_UPDATE_SHADOW_QUEUE_NUM_PARTITIONS)&max_partition_bytes={{ .Values.kafka.maxPartitionBytes }}
{{- else -}}
kafka://$(KAFKA_CLIENT_USER):$(KAFKA_CLIENT_PASSWORD)@$(KAFKA_BROKER_HOST):9092/$(KAFKA_TOPIC_RUN_UPDATE_SHADOW_QUEUE)?consumer_group_id={{ .Values.kafka.consumerGroupId }}&num_partitions=$(KAFKA_RUN_UPDATE_SHADOW_QUEUE_NUM_PARTITIONS)&max_partition_bytes={{ .Values.kafka.maxPartitionBytes }}
{{- end -}}
{{- end -}}
...
{{/* TODO(dpanzella) - Probably need to make this support kafka as well*/}}
{{- define "wandb.fileStreamStoreConsumer" -}}
{{- if .Values.global.pubSub.enabled -}}

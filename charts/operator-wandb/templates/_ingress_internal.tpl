...
It expects a dictionary with three entries:
  ...
  - `local` which contains local ingress settings, e.g. .Values.ingress_internal
  - `context` which is the parent context (either `.` or `$`)

Example usage:
{{- $ingressCfg := dict "global" .Values.global.ingress "local" .Values.ingress_internal "context" . -}}
